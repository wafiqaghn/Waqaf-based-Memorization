...
import { ACTIONS, EVENTS, STATUS, Step, EventHandler, Controls } from 'react-joyride';
...
export const OnboardingProvider = React.memo(({ children }: { children: React.ReactNode }) => {
  ...
  const [joyride, setJoyride] = useState<Controls | null>(null);
  ...
  const steps = useMemo(() => {
    const result = allSteps[activeStep.group].map((s) => {
      const {
        disableBeacon,
        disableOverlay,
        disableOverlayClose,
      ...
      return {
        ...
        skipBeacon: disableBeacon,
        hideOverlay: disableOverlay,
        skipScroll: disableScrolling,
        overlayClickAction: disableOverlayClose ? false : undefined,
        blockTargetInteraction: spotlightClicks ? false : undefined,
        data: {
          ...step.data,
          showNextButton,
          showPrevButton,
  ...
  const joyrideCallback: EventHandler = (data, controls) => {
    setJoyride(controls);
