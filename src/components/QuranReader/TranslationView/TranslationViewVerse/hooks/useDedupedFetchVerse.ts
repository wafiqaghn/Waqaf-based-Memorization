...
const useDedupedFetchVerse = ({
  ...
  const shouldUseHydrationFallback =
    pageNumber === 1 && !isPersistGateHydrationComplete && hasInitialVerses;
  ...
  const { data: verses } = useSWRImmutable(requestKey, verseFetcher, {
    ...
    fallbackData:
      shouldUseInitialData || shouldUseHydrationFallback ? initialData.verses : undefined,
  });
  ...
  const effectiveVerses =
    verses || (shouldUseInitialData || shouldUseHydrationFallback ? initialData.verses : null);
